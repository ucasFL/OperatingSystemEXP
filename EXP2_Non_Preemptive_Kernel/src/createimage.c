...
void extended_opt(Elf32_Ehdr *boot_ehdr, Elf32_Ehdr *kernel_ehdr, 
			Elf32_Phdr *boot_phdr,  Elf32_Phdr *kernel_phdr, int cb, int ck, int cp1, int cp2, int cp3){
...
int main(int argc, char *argv[]){
	if (argc < 3)
	  exit(0);
	Elf32_Ehdr *ehdr, *ehdrk, *ep1, *ep2, *ep3;
	Elf32_Phdr *phdr, *phdrk, *pp1, *pp2, *pp3;
	FILE *fp, *fpk, *p1, *p2, *p3;
	...
	int ker_sec, p1_sec, p2_sec, p3_sec, boot_sec;
	...
	uint8_t zerok[512 * (128 - boot_sec - ker_sec)];
	for ( int i = 0; i < 512 * (128 - boot_sec - ker_sec); i++ )
	  zerok[i] = 0;
	fwrite(zerok, 512 * ( 128 - boot_sec - ker_sec ), 1, imfp );

	int temp1, temp2, temp3;
	temp1 = temp2 = temp3 = 0;
	write_kernel(&imfp, p1, ep1, pp1, &temp1 );
	uint8_t zerop1[512 * (128 - p1_sec)];
	for ( int i = 0; i < 512 * (128 -p1_sec); i++ )
	  zerop1[i] = 0;
	fwrite(zerop1, 512 * (128 - p1_sec), 1, imfp );

	//fseek(imfp, 0, SEEK_END);

	write_kernel(&imfp, p2, ep2, pp2, &temp2);
	uint8_t zerop2[512 * (128 - p2_sec)];
	for ( int i = 0; i < 512 * (128 -p2_sec); i++ )
	  zerop2[i] = 0;
	fwrite(zerop2, 512 * (128 - p2_sec), 1, imfp );

	
	write_kernel(&imfp, p3, ep3, pp3, &temp3);
	record_kernel_sectors( &imfp, 128 * 3 - 1 + p3_sec );

	if(!strcmp(argv[1], "--extended"))
	  extended_opt(ehdr, ehdrk, phdr, phdrk, cb, ck, p1_sec, p2_sec, p3_sec );
